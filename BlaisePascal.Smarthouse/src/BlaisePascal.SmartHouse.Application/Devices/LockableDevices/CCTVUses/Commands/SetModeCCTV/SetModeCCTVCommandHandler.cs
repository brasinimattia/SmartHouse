...
{
    public sealed class SetModeCCTVComanndHandler : IRequestHandler<SetModeCCTVCommand, Result<Guid>>
    {
        private readonly ICCTVRepository _cctvRepository;
        public SetModeCCTVComanndHandler(ICCTVRepository cctvRepository)
        {
            _cctvRepository = cctvRepository;
        }
        public Task<Result<Guid>> Handle(SetModeCCTVCommand request, CancellationToken cancellationToken)
        {
            var cctv = _cctvRepository.GetById(request.Id);
            if (cctv == null)
                return Task.FromResult(Result.Failure<Guid>(Error.NullValue));
            var result = cctv.SetMode(request.mode);
            if (result.IsFailure)
            {
